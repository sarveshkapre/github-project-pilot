...
program
  .command("publish")
  .description("Create GitHub Issues from a summary CSV and issue drafts.")
  ...
  .option("--token-env <name>", "token environment variable", "GITHUB_TOKEN")
  .option("--dry-run", "print intended API calls without publishing", false)
  .action(async (options) => {
    const token = process.env[options.tokenEnv];
    if (!options.dryRun && !token) {
      throw new Error(`Missing token in env ${options.tokenEnv}`);
    }
    ...
    if (options.dryRun) {
      payloads.forEach((payload) => {
        console.log(`[dry-run] ${options.repo}: ${payload.title} (${payload.labels.join(", ")})`);
      });
      return;
    }

    for (const payload of payloads) {
      await createGitHubIssue(options.repo, token ?? "", payload);
    }
  });
...
async function createGitHubIssue(repo: string, token: string, issue: IssueDraft): Promise<void> {
